...
1. **Lambda cold-start and init failure investigator** _(Medium)_
   - Correlate runtime, package/image size, VPC attachment, memory,
     architecture, init duration metrics, timeout, and recent init/error log
     patterns.
   - Output likely causes, confidence, and next checks without returning
     payloads, environment values, or secret values.

2. **Lambda timeout root-cause analyzer** _(Medium)_
   - Combine timeout config, max duration metric, recent timeout log
     fingerprints, downstream dependency hints, VPC/network posture, and event
     source retry settings.
   - Distinguish compute-bound, network-bound, downstream-bound, and
     queue-induced timeout risks without reading payloads or secret values.

3. **SQS backlog stall investigator** _(Medium)_
   - Correlate visible queue depth, oldest message age, Lambda mapping state,
     visibility timeout, Lambda throttles, reserved concurrency, batch
     size/window, DLQ redrive, and partial batch response.
   - Explain the first likely delivery bottleneck without receiving or reading
     queue messages.

4. **EventBridge target retry and DLQ safety audit** _(Medium)_
   - Expand EventBridge delivery diagnostics with retry policy, max event age,
     target DLQ coverage, DLQ queue policy, KMS hints, and failed-invocation
     metrics.
   - Flag events likely to be silently dropped without returning raw policy
     documents or event payloads.

5. **CloudWatch alarm coverage gap finder** _(Medium)_
   - For a Lambda, queue, EventBridge rule, or API Gateway route, inspect
     existing alarms and flag missing coverage for errors, throttles,
     age/backlog, DLQ depth, failed invocations, and latency.
   - Return suggested alarm dimensions and rationale, not alarm-creation
     commands or mutating actions.

6. **IAM explicit deny explainer** _(Medium)_
...
7. **Lambda deployment drift investigator** _(Medium)_
...
8. **API Gateway authorizer failure analyzer** _(Medium)_
...
9. **Step Functions retry/catch safety audit** _(Medium)_
...
10. **DynamoDB stream-to-Lambda readiness check** _(Medium)_
...
11. **S3 notification-to-destination readiness check** _(Medium)_
...
12. **SNS fanout delivery readiness audit** _(Medium)_
...
13. **KMS key lifecycle blast-radius finder** _(Medium)_
...
14. **Log signal correlation timeline** _(Large)_
...
15. **Application dependency graph exporter** _(Large)_
...
16. **First-blocked-edge incident runner** _(Large)_
...
17. **Resource policy condition mismatch analyzer** _(Large)_
...
18. **Multi-region drift and failover readiness audit** _(Large)_
...
19. **Queue/DLQ replay readiness analyzer** _(Large)_
...
20. **Application health narrative generator** _(XLarge)_
